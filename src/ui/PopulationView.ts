...
export class PopulationView extends RenderGroup {
	...
	private resultTxt = new Text("", 225, -30, "middle", new Font("Arial", 30, "normal", FontWeight.Bold));

	constructor(public population: Population = new Population(Consts.POPULATION_SIZE)) {
		super();
		this.populationBox = new PopulationBox(population, 20, 20, -40, -90);
...
export class PopulationBox implements Renderable {
	...
	render(ctx: Context2D) {
		...
		ctx.drawRect(0, 0, bounds.x, bounds.y, false, true);
	...
	private renderCreaturePreview(ctx: Context2D, bounds: Vec2, id: number) {
		...
		ctx.translate(x, y);
		ctx.drawRect(0, 0, BOX_WIDTH, BOX_HEIGHT, false, true);
